...
func postServerBackup(c *gin.Context) {
	...
	case backup.LocalBackupAdapter:
		adapter = backup.NewLocal(client, backupUuid, data.Ignore, s.ID())
	case backup.S3BackupAdapter:
		adapter = backup.NewS3(client, backupUuid, data.Ignore, s.ID())
	default:
...
func deleteServerBackup(c *gin.Context) {
	...
	b, _, err := backup.LocateLocal(middleware.ExtractApiClient(c), backupUuid, s.ID())
	if err != nil {
